        ...
        <div className={`mt-auto pt-2 grid grid-cols-2 gap-2 ${isStacking ? 'opacity-20' : ''}`}>
            {note.audioUrl && (
                <button
                    ...
                    className="col-span-2 bg-sky-400/50 text-sky-800 w-full py-1 sm:py-1.5 rounded-lg text-sm sm:text-base flex items-center justify-center gap-1 sm:gap-2 hover:bg-sky-400 transition duration-200 disabled:bg-sky-200"
                    aria-label="Transcribe audio to text with AI"
                >
                    {isTranscribing ? ( <><LoaderIcon className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" /> Transcribing...</> ) 
                    : ( <><TranscribeIcon className="w-4 h-4 sm:w-5 sm:h-5" /> Transcribe</> )}
                </button>
            )}
            {noteHasText && !note.summary && (
                <button
                    ...
                    className="bg-amber-400/50 text-amber-800 py-1 sm:py-1.5 rounded-lg text-sm sm:text-base flex items-center justify-center gap-1 sm:gap-2 hover:bg-amber-400 transition duration-200 disabled:bg-amber-200"
                    aria-label="Summarize note with AI"
                >
                    {isSummarizing ? ( <><LoaderIcon className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" /> ...</> ) 
                    : ( <><SparklesIcon className="w-4 h-4 sm:w-5 sm:h-5" /> Sum'ize</> )}
                </button>
            )}
            {noteHasText && (
                <button
                    ...
                    className="bg-rose-400/50 text-rose-800 py-1 sm:py-1.5 rounded-lg text-sm sm:text-base flex items-center justify-center gap-1 sm:gap-2 hover:bg-rose-400 transition duration-200 disabled:bg-rose-200"
                    aria-label="Expand note with AI"
                >
                    {isExpanding ? ( <><LoaderIcon className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" /> ...</> ) 
                    : ( <><WandIcon className="w-4 h-4 sm:w-5 sm:h-5" /> Expand</> )}
                </button>
            )}
            {noteHasText && (
                <button
                    ...
                    className="bg-sky-400/50 text-sky-800 py-1 sm:py-1.5 rounded-lg text-sm sm:text-base flex items-center justify-center gap-1 sm:gap-2 hover:bg-sky-400 transition duration-200 disabled:bg-sky-200"
                    aria-label="Find tasks in note with AI"
                >
                    {isExtractingTasks ? ( <><LoaderIcon className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" /> ...</> ) 
                    : ( <><ClipboardListIcon className="w-4 h-4 sm:w-5 sm:h-5" /> Tasks</> )}
                </button>
            )}
            <button
                onClick={() => onFindRelatedNotes(note.id)}
                disabled={isFindingLinks || isStacking}
                className="bg-violet-400/50 text-violet-800 py-1 sm:py-1.5 rounded-lg text-sm sm:text-base flex items-center justify-center gap-1 sm:gap-2 hover:bg-violet-400 transition duration-200 disabled:bg-violet-200"
                aria-label="Find related notes with AI"
            >
                {isFindingLinks ? ( <><LoaderIcon className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" /> ...</> ) 
                : ( <><LinkIcon className="w-4 h-4 sm:w-5 sm:h-5" /> Related</> )}
            </button>
            <button
                onClick={() => onStartStack(note.id)}
                disabled={isStacking}
                className={`py-1 sm:py-1.5 rounded-lg text-sm sm:text-base flex items-center justify-center gap-1 sm:gap-2 transition duration-200 ${
            ...
            >
                <LayersIcon className="w-4 h-4 sm:w-5 sm:h-5" />
